...
router.post('/register', function(req, res, next) {
	...
	if (password !== rePassword) {
		res.render('register', {error: 'The password does not match', username: username});
	} else if (password.length < 8) {
		res.render('register', {error: 'The password is too short, needs 8 characters or more', username: username});
	} else {
...
router.post('/change-password', function(req, res, next) {
	...
	if (newPassword !==  rePassword) {
		res.render('user/change-password', {username:username, error:'The password does not match'});
	} else if (newPassword.length < 8) {
		res.render('user/change-password', {username:username, error:'The password is too short, needs 8 characters or more'});
	} else {
		userManager.setPassword(connection, userID, newPassword, function(result) {
			if (result.code !== 0) {
				res.render('user/change-password', {username:username, error:'Error'});
			} else {
				res.redirect('/users/information');
			}
		});
