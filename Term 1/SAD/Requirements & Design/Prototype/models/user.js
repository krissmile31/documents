const userManager = {
	register:function(connection, name, address, username, password, callback) {
		...
		connection.query(
			...
			function (error, results, fields) {
				if (error) {
					callback({code: -1, message: 'This account has already existed'});
				} else {
					connection.query(
						...
						function(error, results, fields) {
							...
							connection.query(
								...
								accountInfo,
								function (error, results, fields) {
									callback({code: accountInfo.id, message: 'Success'});
								});
						});
				}
			});
		...
		connection.query(
			...
			function (error, results, fields) {
				if (error || results.length <= 0 || results[0].password !== password) {
					...
					console.log(error);
					callback({code:-1, message:'Wrong username or password'});
				} else {
	...
	getInformation:function(connection, userID, callback) {
		connection.query(
