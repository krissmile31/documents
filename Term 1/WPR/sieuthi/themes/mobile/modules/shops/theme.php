					...
					if ($pro_config['active_price'] == '1') {
						if ($data_row_i['showprice'] == '1') {
							...
							if ($data_row_i['product_discounts'] != 0) {
								...
								$xtpl -> assign('product_discounts', CurrencyConversion($data_row_i['product_discounts'] , $data_row_i['money_unit'], $pro_config['money_unit']));
			...
			if ($pro_config['active_price'] == '1') {
				if ($data_row['showprice'] == '1') {
					...
					if ($data_row['product_discounts'] != 0) {
						...
						$xtpl -> parse('main.items.discounts1');
					} else {
						$xtpl -> assign('class_money', 'money');
					}
...
function view_search_all($data_content, $html_pages = "") {
	...
	if (!empty($data_content)) {
		...
		foreach ($data_content as $data_row) {
			...
			if ($pro_config['active_price'] == '1') {
				if ($data_row['showprice'] == '1') {
					$xtpl -> assign('product_price', number_format($data_row['product_price'], 0, '.', ' ') . " " );
					if ($data_row['product_discounts'] != 0) {
						//$price_product_discounts = $data_row['product_price'] - ($data_row['product_price'] * ($data_row['product_discounts'] / 100));
						$price_product_discounts = $data_row['product_discounts'];
						$xtpl -> assign('product_discounts', number_format($price_product_discounts, 0, '.', ' ') . " " );
					...
					if ($data_row['product_discounts'] != 0) {
						...
						$xtpl -> assign('class_money', 'color:#FF0000; font-size:18px');
						$xtpl -> parse('main.items.price1.discounts');
					} else {
						$xtpl -> assign('class_money', 'color:#999999');
					}
					$xtpl -> parse('main.items.price1');
				} else {
					$xtpl -> parse('main.items.contact');
...
function viewcat_page_gird($data_content, $pages) {
	...
	{
		if($array_sort_i==$sort_id) $select ="selected='selected'";
		else $select="";
				...
				if ($data_row['showprice'] == '1') {
					...
					if ($data_row['product_discounts'] != 0) {
						...
						if ($pro_config['active_price'] == '1')
							...
							$xtpl -> parse('main.grid_rows.discounts1');
					} else {
						$xtpl -> assign('class_money', 'money');
					}
...
function viewcat_page_list($data_content, $pages) {
	global $module_info, $lang_module, $module_file, $module_name, $pro_config, $sort_id,$array_op, $global_array_cat;
	...
	foreach($array_sort as $array_sort_i=>$value)
	{
		if($array_sort_i+1==$sort_id) $select ="selected='selected'";
		else $select="";
		$xtpl->assign('select',$select);
		$xtpl->assign('value',$value);
			...
			if ($pro_config['active_price'] == '1') {
				if ($data_row['showprice'] == '1') {
					...
					if ($data_row['product_discounts'] != 0) {
						...
						$xtpl -> parse('main.row.discounts1');
					} else {
						$xtpl -> assign('class_money', 'money');
					}
...
function detail_product($data_content, $data_unit, $data_comment, $num_comment, $data_others, $data_shop, $array_other_view) {
	global $module_info, $lang_module, $module_file, $global_config, $module_name, $my_head, $pro_config, $module_data, $array_op,$global_array_source;
	...
	if (!empty($data_content)) {
		...
		$xtpl -> assign('keywords', $data_content['vi_keywords']);
		...
		$a= round ((100-(($data_content['product_discounts'])/($data_content['product_price']))*100));
			//print_r($a);exit();
			if
			($a != 100)
			{
		...
		if ($data_content['product_discounts'] != 0) {
			...
			$xtpl -> assign('class_money', 'discounts2_money');
			...
			$xtpl -> assign('class_money', 'money2');
		}
