...
if (!nv_function_exists('nv_message_slider')) {

	function nv_block_config_message_slider2_blocks($module, $data_block, $lang_block) {
	...
	function nv_block_config_message_slider2_blocks_submit($module, $lang_block) {
		global $nv_Request;
		$return = array();
		$return['error'] = array();
		$return['config'] = array();
	...
	function nv_message_slider2($block_config) {
    ...
    $i=1;
		...
		foreach ($array_th as $row) {
			$row['bodytext'] = nv_clean60($row['bodytext'], 80);
...
if (defined('NV_SYSTEM')) {
	$content = nv_message_slider2($block_config);
}
?>