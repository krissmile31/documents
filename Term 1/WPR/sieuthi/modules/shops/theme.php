        ...
        foreach ( $data_content as $data_row )
        {
            if ( $data_row['num_pro'] > 0 )
            {
                ...
                foreach ( $data_row['data'] as $data_row_i )
                {
					...
					$xtpl->assign( 'publtime', nv_date( "d/m/Y g:i a", $data_row_i['publtime'] ) );
					...
					{	
						if($data_row_i['product_discounts'] != 0 and $data_row_i['showprice'] == '1')
						{
							$xtpl->parse( 'main.catalogs.items.tooltip.khuyenmai' );
						}
						if($data_row_i['showprice'] == '1')
						{
							$xtpl->parse( 'main.catalogs.items.tooltip.gia' );

						}else{
							$xtpl->assign( 'lienhe', 'Liên Hệ' );
							$xtpl->parse( 'main.catalogs.items.tooltip.lienhe' );
						}
						if($data_row_i['promotional']!="")
						{
							$xtpl->assign( 'phukien', $data_row_i['promotional'] );
							$xtpl->parse( 'main.catalogs.items.tooltip.phukien' );
						}
						if($data_row_i['address']!="")
						{
							$xtpl->assign( 'address', $data_row_i['address'] );
							$xtpl->parse( 'main.catalogs.items.tooltip.address' );
						}
						if($data_row_i['warranty']!="")
						{
							$xtpl->assign( 'warranty', $data_row_i['warranty'] );
							$xtpl->parse( 'main.catalogs.items.tooltip.warranty' );
						}
					...
					// add moi
					if((NV_CURRENTTIME < $data_row_i['pricetime']) and ($data_row_i['khuyenmai'] == 1 or $data_row_i['khuyenmai'] == 2 or $data_row_i['khuyenmai'] == 3 or $data_row_i['khuyenmai'] == 4))
					{
							if($data_row_i['khuyenmai'] == 1){
								$xtpl->parse( 'main.catalogs.items.hangmoi' );
							}
							if($data_row_i['khuyenmai'] == 2){
								$xtpl->parse( 'main.catalogs.items.hanghot' );
							}
							if($data_row_i['khuyenmai'] == 3){
								$xtpl->parse( 'main.catalogs.items.noibat' );
							}
							if($data_row_i['product_discounts'] != 0 ){
								$xtpl->assign( 'discounts', $data_row_i['product_discounts'] );
								$xtpl->parse( 'main.catalogs.items.giamgia' );
							}
							/*
							if($data_row_i['khuyenmai'] == 4 and $data_row_i['product_discounts'] != 0 ){
								$xtpl->assign( 'discounts', $data_row_i['product_discounts'] );
								$xtpl->parse( 'main.catalogs.items.giamgia' );
							}
							*/
...
function view_home_all ( $data_content ,$html_pages ="")
{
    ...
    if ( ! empty( $data_content ) )
    {
        ...
        foreach ( $data_content as $data_row )
        {
			...
			//$xtpl->assign( 'publtime', nv_date( "d/m/Y g:i a", $data_row['publtime'] ) );
            $pwidth = ( int )( 100 / $num_view );
            if ( $i % $pro_config['per_row'] == 0 )
            {
            ...
            if ( $pro_config['active_price'] == '1' )
            {
                if ( $data_row['showprice'] == '1' )
                {
                    ...
                    if ( $data_row['product_discounts'] != 0 )
                    {
                        ...
                        $xtpl->assign( 'class_money', 'color: #999999; text-decoration:line-through' );
                        $xtpl->parse( 'main.items.price.discounts' );
                    }
                    else
                    {
                        $xtpl->assign( 'class_money', 'color:#993300' );
                    }
...
function detail_product ( $data_content, $data_unit, $data_comment, $num_comment, $data_others, $data_shop, $array_other_view )
{
    ...
    if ( ! empty( $data_content ) )
    {
        ...
        if ( $data_content['product_discounts'] != 0 )
        {
            ...
            $xtpl->assign( 'class_money', 'color:#666666; text-decoration:line-through' );
            $xtpl->parse( 'main.price.discounts' );
        }
        else
        {
            $xtpl->assign( 'class_money', 'color:#993333; font-size: 16px' );
        }
