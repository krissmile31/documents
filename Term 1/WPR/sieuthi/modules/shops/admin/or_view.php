...
$listcolor = explode( "|", $data_content['listcolor'] );
$data_pro = array();
$i = 0;
foreach ( $listid as $id )
{
    ...
    $data_pro[] = array( 
        "id" => $id, "publtime" => $publtime, "title" => $title, "alias" => $alias, "product_price" => $listprice[$i], "product_unit" => $unit, "link_pro" => $link . "detail/" . $id . "/" . $alias, "product_number" => $listnum[$i],"color" => $listcolor[$i] 
    );
    $i ++;
}
...
foreach ( $data_pro as $pdata )
{
	...
	if(!empty($pdata['color'])) 
		$xtpl->assign('product_color',$global_array_color[$pdata['color']]['title']);
	else
		$xtpl->assign('product_color','---');
