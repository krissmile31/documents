...
$rowcontent = array("id" => 0, "listcatid" => $catid,"color_id"=>"", "topic_id" => "", "group_id" => "", "user_id" => $admin_info['admin_id'], "source_id" => 0, 'shopcat_id' => 0, 'com_id' => 0, "addtime" => NV_CURRENTTIME, "edittime" => NV_CURRENTTIME, "status" => 0, "publtime" => NV_CURRENTTIME, "exptime" => 0, "archive" => 1, "product_number" => 1, "product_price" => 1, "product_discounts" => 0, "money_unit" => "", "product_unit" => "", "homeimgfile" => "", "homeimgthumb" => "", "homeimgalt" => "", "imgposition" => 0, "copyright" => 0, "inhome" => 1, "allowed_comm" => "", "allowed_rating" => 1, "ratingdetail" => "0", "allowed_send" => 1, "allowed_print" => 1, "allowed_save" => 1, "hitstotal" => 0, "hitscm" => 0, "hitslm" => 0, "showprice" => 1, "com_id" => 0, "title" => "", "alias" => "", "hometext" => "", "bodytext" => "", "note" => "", "keywords" => "", "address" => "", "codesp" => "", "description" => "", "warranty" => "", "promotional" => "","detailpromotional"=>"","maps"=>"");
...
if ($nv_Request -> get_int('save', 'post') == 1) {
	...
	//color size
	$color_id = array_unique( $nv_Request->get_typed_array( 'colorids', 'post', 'int', array() ) );
    $rowcontent['color_id'] = implode( ",", $color_id );
	...
	$rowcontent['hometext'] = $nv_Request->get_string( 'hometext', 'post', '' );
	...
	$rowcontent['maps'] = filter_text_input( 'maps', 'post', '', 1 );
	$rowcontent['codesp'] = filter_text_input('codesp', 'post', '', 1);
	...
	$detailpromotional = $nv_Request -> get_string('detailpromotional', 'post', '');
	$rowcontent['detailpromotional'] = defined('NV_EDITOR') ? nv_nl2br($detailpromotional, '') : nv_nl2br(nv_htmlspecialchars(strip_tags($detailpromotional)), '<br />');

	
	$sourcetext = filter_text_input('sourcetext', 'post', '', 1);
	$rowcontent['copyright'] = $nv_Request -> get_int('copyright', 'post',0);
	$rowcontent['inhome'] = $nv_Request -> get_int('inhome', 'post',0);
	$rowcontent['allowed_comm'] = $nv_Request -> get_int('allowed_comm', 'post', 0);
	$rowcontent['allowed_rating'] = $nv_Request -> get_int('allowed_rating', 'post',0);
	$rowcontent['allowed_send'] = $nv_Request -> get_int('allowed_send', 'post',0);
	$rowcontent['allowed_print'] = $nv_Request -> get_int('allowed_print', 'post',0);
	$rowcontent['allowed_save'] = $nv_Request -> get_int('allowed_save', 'post',0);
	$rowcontent['shopcat_id '] = 0;
	...
	if (empty($rowcontent['title'])) {
		...
		if ($rowcontent['id'] == 0) {
			...
			$query = "UPDATE `" . $db_config['prefix'] . "_" . $module_data . "_rows` SET 
						   ...
						   `color_id`=" . $db->dbescape_string( $rowcontent['color_id']."," ) . ",
                           ...
                           `" . NV_LANG_DATA . "_codesp`=" . $db -> dbescape_string($rowcontent['codesp']) . ", 
                           ...
                           `" . NV_LANG_DATA . "_warranty`=" . $db -> dbescape_string($rowcontent['warranty']) . ",
                       	   `" . NV_LANG_DATA . "_detailpromotional`=" . $db -> dbescape_string($rowcontent['detailpromotional']) . ",
					   	   `" . NV_LANG_DATA . "_maps`=" . $db -> dbescape_string($rowcontent['maps']) . "
					    WHERE `id` =" . $rowcontent['id'] . "";
			$db -> sql_query($query);
			if ($db -> sql_affectedrows() > 0) {
				nv_fix_group_count($rowcontent['group_id']);
				if ($rowcontent_old['group_id'] != $rowcontent['group_id'])
					nv_fix_group_count($rowcontent_old['group_id']);
...
if (!empty($rowcontent['detailpromotional']))
	$rowcontent['detailpromotional'] = nv_htmlspecialchars($rowcontent['detailpromotional']);	
...
//print_r(NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/" . $module_file);exit();
...
while (list($catid_i, $title_i, $lev_i, $numsubcat_i) = $db -> sql_fetchrow($result_cat)) {
	if ($rowcontent['listcatid'] == 0)
		$rowcontent['listcatid'] = $catid_i;
	$xtitle_i = "";
	if ($lev_i > 0) {
		for ($i = 1; $i <= $lev_i; $i++) {
			$xtitle_i .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
...
if (defined('NV_EDITOR') and function_exists('nv_aleditor')) {
	...
	$detailpro = nv_aleditor('detailpromotional', '100%', '300px', $rowcontent['detailpromotional']);
} else {
	$edits = "<textarea style=\"width: 100%\" name=\"bodytext\" id=\"bodytext\" cols=\"20\" rows=\"15\">" . $rowcontent['bodytext'] . "</textarea>";
	$detailpro = "<textarea style=\"width: 100%\" name=\"detailpromotional\" id=\"detailpromotional\" cols=\"20\" rows=\"15\">" . $rowcontent['detailpromotional'] . "</textarea>";
}
///////////////////////////////////////////////////////////////////////////////////////////
$shtm = "";
...
if ( count( $array_color_cat ) > 0 )
{
    ...
    {
        $cch = in_array( $cid_i, $color_id ) ? " checked=\"checked\"" : "";
        $colorhtm .= "<input class=\"news_checkbox\" type=\"checkbox\" name=\"colorids[]\" value=\"" . $cid_i . "\"" . $cch . ">" . $cid_title . "<br />\n";
    }
...
$xtpl->assign( 'maps', "value='" . $rowcontent['maps'] . "' name='maps'" );
////////////////////////////////////////////////////////////////////////////////
$xtpl -> assign('edit_bodytext', $edits);
$xtpl -> assign('detailpro', $detailpro);
///////////////////////////////////////////////////////////////////////////////////
if ($rowcontent['id'] > 0) {
