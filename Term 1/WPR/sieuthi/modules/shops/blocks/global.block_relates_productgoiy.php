...
if ( ! nv_function_exists( 'nv_relates2_product' ) )
{

    function nv_block_config_relates2_blocks ( $module, $data_block, $lang_block )
    {
    ...
    function nv_block_config_relates2_blocks_submit ( $module, $lang_block )
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_relates2_product ( $block_config )
    {
        ...
        /*get theme block*/
        if ( file_exists( NV_ROOTDIR . "/themes/" . $global_config['site_theme'] . "/modules/" . $mod_file . "/block.others_productgoiy.tpl" ) )
        {
		...
		$xtpl = new XTemplate( "block.others_productgoiy.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/" . $mod_file );
		...
		foreach ( $list as $l )
        {
			...
			if($y==5 OR $y==14){
		  $xtpl->assign('A','align-right');
		  }
		  else{  $xtpl->assign('A','csmall-tile ');
		  }
...
if ( defined( 'NV_SYSTEM' ) )
{
    global $site_mods;
    $module = $block_config['module'];
    $content = nv_relates2_product( $block_config );
}
