...
if (!function_exists('nv_pro_catalogscate')) {
	function nv_block_config_product_catalogscate_blocks($module, $data_block, $lang_block) {
	...
	function nv_block_config_product_catalogscate_blocks_submit($module, $lang_block) {
		global $nv_Request;
		$return = array();
		$return['error'] = array();
		$return['config'] = array();
	...
	function nv_pro_catalogscate($block_config) {
		global $home, $site_mods, $global_config, $module_config, $module_name, $module_info, $global_array_cat, $db, $db_config, $my_head, $array_cat_shops, $catid;
		...
		if ($module != $module_name) {
			$sql = "SELECT catid, image, icon, parentid, lev," . NV_LANG_DATA . "_title as title," . NV_LANG_DATA . "_alias as alias, viewcat, numsubcat, subcatid, numlinks, del_cache_time, " . NV_LANG_DATA . "_description, inhome, " . NV_LANG_DATA . "_keywords, who_view, groups_view FROM `" . $db_config['prefix'] . "_" . $mod_data . "_catalogscate` ORDER BY `order` ASC";
			//die($sql);
		...
		foreach ($array_cat_shops as $cat) {
			if ($cat['parentid'] == 0 && $cat['catid'] == $catid ) {
				if ($cat['inhome'] == '1') {
		...
		if (empty($list_sub))
			...
			foreach ($list as $catid) {
				if ($array_cat_shops[$catid]['inhome'] == '1') {
					$html .= "<li class=\"list\">\n";
					$html .= "<a class=\"started\" href=\"" . $array_cat_shops[$catid]['link'] . "\">
						<div><img src=\"" . $array_cat_shops[$catid]['icon'] . "\" style=\"height:125px; width:125px;\";></div>
						<h2 class=\"category_feture_name\">" . $array_cat_shops[$catid]['title'] . "</h2>
						<h3>" . $array_cat_shops[$catid]['description'] . "</h3>
					</a>";
...
if (defined('NV_SYSTEM')) {
	global $site_mods;
	$module = $block_config['module'];
	$content = nv_pro_catalogscate($block_config);
}
?>