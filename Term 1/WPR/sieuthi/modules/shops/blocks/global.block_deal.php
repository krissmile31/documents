...
if ( ! nv_function_exists( 'nv_deal' ) )
{

    function nv_block_config_deal_blocks ( $module, $data_block, $lang_block )
    {
    ...
    function nv_block_config_deal_blocks_submit ( $module, $lang_block )
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_deal ( $block_config )
    {
        ...
        /*get theme block*/
        if ( file_exists( NV_ROOTDIR . "/themes/" . $global_config['site_theme'] . "/modules/" . $mod_file . "/block.deal.tpl" ) )
        {
		...
		$xtpl = new XTemplate( "block.deal.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/" . $mod_file );
		$xtpl->assign( 'LANG', $lang_module );
		$xtpl->assign( 'title_price', $lang_module['title_price'] );
		$xtpl->assign( 'detail_product', $lang_module['detail_product'] );
		
        $sql = "SELECT  t1.id, t1.listcatid, t1." . NV_LANG_DATA . "_title as title , t1." . NV_LANG_DATA . "_alias as alias, t1." . NV_LANG_DATA . "_hometext as hometext, t1.addtime,t1.homeimgthumb,t1.product_price,t1.product_discounts,t1.money_unit,t1.showprice FROM `" . $db_config['prefix'] . "_" . $module . "_rows` as t1 INNER JOIN `" . $db_config['prefix'] . "_" . $module . "_block` AS t2 ON t1.id = t2.id WHERE t1.status= 1 AND  t1.publtime < " . NV_CURRENTTIME . " AND (t1.exptime=0 OR t1.exptime >" . NV_CURRENTTIME . ") AND t1.product_discounts > 0 ORDER BY RAND() LIMIT 0 , " . $block_config['numrow'];
		$list = nv_db_cache_adv( $sql, 'dealid'.$block_config['bid'], $module );
        $i = 1;
		foreach ( $list as $l )
		{
            ...
            $xtpl->assign( 'link', $link . $array_cat_shops[$l['listcatid']]['alias'] . "/" . $l['alias'] . "-" . $l['id'] );
            $title_i = nv_clean60( $l['title'], 24 );
