...
function onsubmitsearch_home()
{
    ...
    if ( keyword == '' )
    {
    	...
    	window.location.href = nv_siteroot + 'index.php?' + nv_lang_variable + '=' + nv_sitelang + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=search_result&keyword='+rawurlencode(keyword);
    }
    return false;
}

function changeviewtype(a,b){
	c = document.getElementById('sort').value;
	$.ajax({
        type: "POST",
        url: nv_siteroot + 'index.php?' + nv_lang_variable + '=' + nv_sitelang + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '='+b,
        data: 'view=' + a,
        success: function(data){
			window.location.href = nv_siteroot + 'index.php?' + nv_lang_variable + '=' + nv_sitelang + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '='+b+'&view='+a+'&sort='+c;
        
        }
    });
}
function changesort(x,y,z){
    $.ajax({
        type: "POST",
        url: nv_siteroot + 'index.php?' + nv_lang_variable + '=' + nv_sitelang + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '='+y,
        data: 'sort=' + x,
        success: function(data){
			window.location.href = nv_siteroot + 'index.php?' + nv_lang_variable + '=' + nv_sitelang + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '='+y+'&sort='+x+'&view='+z;   
        }
    });
}
function addclick(node) {
	var listPID = document.getElementById('list_product_compare').value;
	if (node.checked) {
		var tmp = listPID.split(',');
		if (tmp.length >= 4) {
			alert("Bạn chỉ được chọn tối đa 4 sản phẩm cho một lần so sánh");
			node.checked = false;
			return;
		}
		if (listPID.indexOf(node.value) < 0) {
			listPID += (listPID == "" ? "" : ",") + node.value;
		}
	}
	else {
		if (listPID.indexOf(node.value) > 0) {
	...
	document.getElementById('list_product_compare').value = listPID;
}
function compare_pro() {
	...
	if (tmp.length <= 1) {
		...
		window.location.href = nv_siteroot + 'index.php?' + nv_lang_variable + '=' + nv_sitelang + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=compare_pro&list_pro='+tmp; 
	}
}