...
$list = nv_db_cache( $sql, 'money'.NV_LANG_DATA, $module_name );
foreach ( $list as $l )
{
    $is_config = ( $l['code'] == $pro_config['money_unit'] ) ? 1 : 0;
    $money_config[$l['code']] = array( 
        'code' => $l['code'], 'currency' => $l['currency'], 'exchange' => $l['exchange'], "is_config" => $is_config 
    );
}
...
function nv_list_lang ( )
{
	...
	$list = nv_db_cache( $sql, 'setup_language', $module_name );
	$lang_value = array();
	foreach ( $list as $l )
    {
        $lang_value[] = $l['lang'];
