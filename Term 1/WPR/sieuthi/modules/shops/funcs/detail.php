...
$s = "SELECT id, " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias, homeimgthumb,addtime, product_price, hitstotal, product_discounts, money_unit,showprice," . NV_LANG_DATA . "_hometext,homeimgfile  FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE id!= ".$id." AND listcatid = " . $data_content['listcatid'] . " AND inhome=1 AND status=1 AND publtime < " . NV_CURRENTTIME . " AND (exptime=0 OR exptime>" . NV_CURRENTTIME . ") ORDER BY ID DESC LIMIT ".($pro_config['per_row']*2);
...
while ( list( $id, $title, $alias, $homeimgthumb, $addtime, $product_price, $hitstotal, $product_discounts, $money_unit, $showprice, $hometext, $homeimgfile ) = $db->sql_fetchrow( $re ) )

{
    ...
    $data_others[] = array( 
        ...
        "id" => $id, "title" => $title, "alias" => $alias, "homeimgthumb" => $thumb[0], "hometext" => $hometext ,"addtime" => $addtime, "product_price" => $product_price, "hitstotal" => $hitstotal, "product_discounts" => $product_discounts, "money_unit" => $money_unit, "showprice" => $showprice, "link_pro" => $link . $global_array_cat[$data_content['listcatid']]['alias'] . "/" . $alias . "-" . $id, "link_order" => $link . "setcart&amp;id=" . $id , "homeimgfile"=>$homeimgfile
...

{
    ...
    $s = "SELECT id, " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias, homeimgthumb,addtime, product_price, hitstotal, product_discounts, money_unit,showprice, " . NV_LANG_DATA . "_hometext ,homeimgfile FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE id IN ( " . $arrtempid . ") AND inhome=1 AND status=1 AND publtime < " . NV_CURRENTTIME . " AND (exptime=0 OR exptime>" . NV_CURRENTTIME . ") ORDER BY ID DESC LIMIT ".($pro_config['per_row']*2);
