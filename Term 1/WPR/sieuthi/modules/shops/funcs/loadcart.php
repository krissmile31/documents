...
$i=0;
	foreach ($_SESSION[$module_data.'_cart'] as $pro_id => $info) {
		$xtpl -> assign('id', $info['id']);
		$xtpl -> assign('price_p', $info['price']);
		$xtpl -> assign('info_n', $info['num']);
		$xtpl -> assign('title_pro', nv_clean60($info['title'], 70));
		$xtpl -> assign('img_pro', $info['homeimgthumb']);
...
//print_r($i);exit();
 if(empty($_SESSION[$module_data . '_cart'])) {
 $xtpl -> assign('giohangrong', 'Giỏ hàng không có sản phẩm nào!');
 $xtpl -> parse('main.enable1');
