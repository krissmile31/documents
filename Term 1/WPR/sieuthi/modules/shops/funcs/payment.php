...
if ( $order_id > 0 and $checkss == md5( $order_id . $global_config['sitekey'] . session_id() ) )
{
	...
	$listcolor = explode("|", $data['listcolor']);
    $temppro = array();
    $i = 0;
    foreach ( $listid as $proid )
    {
		...
		if ( empty($listcolor[$i]) )
		{
			 $listcolor[$i]=0;
			 $global_array_color[0]['title']="---";
		}
        $temppro[$proid] = array( 
            "price" => $listprice[$i], "num" => $listnum[$i] ,"color" => $global_array_color[$listcolor[$i]]['title']
        );
        $arrayid[] = $proid;
        $i ++;
    }
    ...
    if ( ! empty( $arrayid ) )
    {
        ...
        {
            $data_pro[] = array( 
                "id" => $id, "publtime" => $publtime, "title" => $title, "alias" => $alias, "product_note" => $note, "hometext" => $hometext, "product_price" => $temppro[$id]['price'], "product_unit" => $unit, "money_unit" => $money_unit, "link_pro" => $link . $global_array_cat[$listcatid]['alias'] . "/" . $alias . "-" . $id, "product_number" => $temppro[$id]['num'] ,"color" => $temppro[$id]['color']
            );
        }
    }
