...
if ( ! nv_function_exists( 'nv_news_block_tophits' ) )
{
    ...
    function nv_news_block_tophits ( $block_config, $mod_data )
    {
        ...
        $sql = "SELECT id, listcatid, publtime, exptime, title, hometext, alias, homeimgthumb, homeimgfile FROM `" . NV_PREFIXLANG . "_" . $mod_data . "_rows` WHERE `status`= 1 AND `publtime` BETWEEN  " . $publtime . " AND " . NV_CURRENTTIME . " AND (`exptime`=0 OR `exptime`>" . NV_CURRENTTIME . ") ORDER BY id DESC LIMIT 0 , " . $block_config['numrow'];
        $result = $db->sql_query( $sql );
        while ( list( $id, $listcatid, $publtime, $exptime, $title, $hometext, $alias, $homeimgthumb, $homeimgfile ) = $db->sql_fetchrow( $result ) )
        {
            ...
            $array_block_news[] = array( 
                'id' => $id, 'listcatid' => $listcatid, 'title' => $title,  'hometext' => $hometext,  'link' => $link, 'imgurl' => $imgurl, 'width' => $blockwidth 
            );
        }
        ...
        foreach ( $array_block_news as $array_news )
        {
			if($a==1)
			{
				$xtpl->assign( 'blocknews2', $array_news );
