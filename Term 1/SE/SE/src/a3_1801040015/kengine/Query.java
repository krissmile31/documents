...
public class Query {
  ...
  public String toString() {
	...
	public Iterator matchIterator() {
		if (matches.size() <= 0)
			throw new NotPossibleException("Empty data!");
		
		return new MatchGen();
	}
	
	/**
	 * @overview represents a generator of the elements of a Match.
	 * @atributes
	 * indx 			Integer
	 * @abstract_properties
	 * mutable(indx) = false 		/\  	min(indx) = 0 		/\ indx < Query.size()	/\
	 * MatchGen.new = [x1, ...] where each xi is in Query.Macth and xis are arranged in same order as MatchGen.Query's elements.
	 *
	 */
	private class MatchGen implements Iterator {
		 ...
		 * 		and modifies the state of this to record
		 * else
		...
		public Object next() throws NoSuchElementException {
			if (hasNext() == true) {
				Object next = matches.get(indx);
				indx++;
				return next;
			}
			throw new NoSuchElementException("Query.iterator");
		}
