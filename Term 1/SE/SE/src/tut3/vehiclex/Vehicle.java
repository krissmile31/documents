 ...
 * @attributes name String width Double height Double length Double weight
 *             Double seatingCapacity Integer
 * @object A typical Vehicle is
 * 
 *         <pre>
 * v = < n,d,h,l,w,c >
 *         </pre>
 ...
 * 
 *         <pre>
 * name(n), width(d), height(h), length(l), weight(w), seatingCapacity(c)
 *         </pre>
 * 
 * @abstract_properties 
 * mutable(name)=true /\ optional(name)=false /\
 * mutable(width)=true /\ optional(width)=false /\
 * min(width)=0+ /\ mutable(height)=true /\
 * optional(height)=false /\ min(height)=0+ /\
 * mutable(length)=true /\ optional(length)=false /\
 * min(length)=0+ /\ mutable(weight)=true /\
 * optional(weight)=false /\ min(weight)=0+ /\
 * mutable(seatingCapacity)=true /\
 * optional(seatingCapacity)=false /\
 * min(seatingCapacity)=1 /\
 * @author dmle
 */
public class Vehicle implements Comparable {
	 ...
	 *          </pre>
	 */
	public Vehicle(@AttrRef("name") String n, @AttrRef("width") double d, @AttrRef("height") double h,
			@AttrRef("length") double l, @AttrRef("weight") double w, @AttrRef("seatingCapacity") int c)
			throws NotPossibleException {
		if (!validateName(n))
			throw new NotPossibleException("Vehicle<init>: Invalid name!");
		if (!validateDimension(d))
			throw new NotPossibleException("Vehicle<init>: Invalid width!");
		if (!validateDimension(h))
			throw new NotPossibleException("Vehicle<init>: Invalid height!");
		if (!validateDimension(l))
			throw new NotPossibleException("Vehicle<init>: Invalid length!");
		if (!validateWeight(w))
			throw new NotPossibleException("Vehicle<init>: Invalid weight!");
		if (!validateSeatingCapacity(c))
			throw new NotPossibleException("Vehicle<inint>: Invalid seating!");
	...
	/**
	 * @effects return the total weight (in kilograms) as the sum of weight and the
	 *          estimated passengers weight
	 */
	 ...
	 *          </pre>
	 */
	private boolean validate(String n, double d, double h, double l, double w, int c) {
		return validateName(n) && validateDimension(d) && validateDimension(h) && validateDimension(l)
