...
public class WordTable {
   ...
   * Constructor method
   * 
   * @effects If the file cannot be read throws <code>NotPossibleException</code>,
   *          else initialises the table to contain all the words in the file as
   *          uninteresting words.
  ...
  public WordTable() throws NotPossibleException {
    ...
    BufferedReader bf = new BufferedReader(new InputStreamReader(this.getClass().getResourceAsStream(NK_FILE)));

    try {
      if (!bf.ready())
        throw new NotPossibleException("WordTable.init(): Failed to read non-key file " + NK_FILE);
    } catch (IOException ex) {
      throw new NotPossibleException("WordTable.init(): Failed to read non-key file " + NK_FILE + "due to " + ex);
    }
   ...
   * 
   * @param d a <code>Doc</code> object representing a document
   ...
   *          with a count of their number of occurrences; returns a table mapping
   *          each interesting word in <code>d</code> to its number of
   *          occurrences.
   * 
   * @version 3.0
   */
