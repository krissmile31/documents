...
public class Query {
	...
	/**
	 * @requires matches.elements must not be modified during generator operation
	 * @effect
	 * 
	 *         <pre>
	 *  if matches.elements is empty
	...
	public Iterator matchIterator() {
		if (matches.size() <= 0) {
			throw new NotPossibleException("Query.matchIterator(): Data empty");
		} else {
	 ...
	 * <pre>
	  * &#64;overview
	  *  	Query.MatchGenerator represents a generator of the elements of <tt>matches</tt>
	  * &#64;attributes
	  *  index Integer
	  * &#64;abstract_properties
	  *  MatchGenerator.new = [x1, x2, ...xn] such that each <tt>xi</tt> is a match in <tt>Query.matches</tt>
	  * &#64;abstract_function
	  *  <tt>AF(c) = [x1, x2, ... xn]</tt> such that each <tt>xi</tt> is in <tt>Query.matches</tt>
	  * &#64;rep_invariant
	  *     index >= 0 /\ index < <tt>matches.size()</tt>
	 * </pre>
	 */
	public class MatchGenerator implements Iterator {

		private int index;

		/**
		 * initializes position to be at the beginning of set
		 */
		 ...
		 * <pre>
		 * &#64;effect
		 *  if index >= matches.size() or matches.size() == 0
		 *      return false
		 *  else
		 *      return true
		 * &#64;return
		...
		public boolean hasNext() {
			return (matches.size() != 0 && index < matches.size());
		}

		/**
		 * @effect if hasNext() return item at index else throws NotPossibleException
		 * @return
		 */
		public Object next() {
			if (hasNext()) {
				Object next = matches.get(index);
				index++;
				return next;
			}

			throw new NotPossibleException("Query.MatchGenerator: no item");
