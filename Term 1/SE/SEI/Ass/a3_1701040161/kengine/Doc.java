...
public class Doc {
   ...
   * 
   * @effects returns a generator that will yield all the words in the document as
   *          strings in the order they appear in the text
   *          <p>
   * 
        ...
        if (skip) {
          ...
          if (c == CLOSED_TAG) { // end of opening tag, beginning text
            ...
            if (SKIP_SPECIAL_HTML) {
              ...
              if (!(reading.startsWith("script") || reading.startsWith("SCRIPT") || reading.startsWith("style")
   ...
   *           to return a word iterator of a document.
   * 
   * @author dmle
   *
   */
  class WordGenerator implements Iterator {
    ...
    /**
     * @effects throws <code>NotImplementedException<code> because <code>Doc</code>
     *          is immutable
     * 
     */
    public void remove() throws NotImplementedException {
      // not implemented
      throw new NotImplementedException("WordGenerator.remove: Doc is immutable");
    }
  }
}
